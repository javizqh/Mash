...
int
kill_job(int argc, char *argv[], int stdout_fd, int stderr_fd)
{
	argc--;
	argv++;
	...
	if (signal == SIGTSTP && signal != SIGSTOP && signal != SIGTTIN
	    && signal != SIGTTOU) {
		stop_job(job->pid);
